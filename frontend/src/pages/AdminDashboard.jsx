...
const AdminDashboard = () => {
    ...
    if (!user || user.role !== 'ADMIN') {
        return (
            <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-4">
                <div className="max-w-md w-full bg-slate-900/50 p-8 rounded-2xl border border-red-900/50 backdrop-blur-xl text-center">
                    ...
                    <p className="mb-8 text-slate-400">Classified Level 5 Clearance Required.</p>
    ...
    const SidebarItem = ({ id, icon: Icon, label }) => (
        <button
            ...
            className={`w-full flex items-center gap-4 px-6 py-4 transition-all ${activeTab === id ? 'bg-indigo-600/20 text-indigo-400 border-r-4 border-indigo-500' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}
        >
    ...
    return (
        <div className="min-h-screen bg-black text-slate-200 flex overflow-hidden font-sans">
            {/* Sidebar */}
            <div className="w-64 bg-slate-900/50 border-r border-white/10 flex flex-col backdrop-blur-xl h-screen sticky top-0">
                <div className="p-8">
                    ...
                    <p className="text-xs text-slate-500 tracking-widest mt-1">V2.0.4 BETA</p>
                </div>
                    ...
                    <div className="flex items-center gap-3 px-4 py-3 bg-white/5 rounded-xl border border-white/5">
                        ...
                        <div className="flex items-center gap-4">
                            <button
                                ...
                                disabled={loading}
                                className={`p-3 rounded-xl border border-white/10 text-slate-300 hover:text-white hover:bg-white/5 transition-all ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
                ...
                <div className="p-8 max-w-7xl mx-auto pb-24">
                    ...
                    <header className="mb-8 flex justify-between items-end">
                        <div>
                            ...
                            <p className="text-slate-400">System Monitoring & Control Interface</p>
                        </div>
                        <div className="text-right hidden md:block">
                            <p className="text-xs text-slate-500 font-mono">SESSION ID: {Math.random().toString(36).substr(2, 9).toUpperCase()}</p>
                    ...
                    {!loading && activeTab === 'overview' && (
                        <div className="space-y-6">
                            {/* Traffic Chart */}
                            <div className="bg-slate-900/50 p-6 rounded-3xl border border-white/10 backdrop-blur-sm h-80 relative overflow-hidden group">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div className="bg-slate-900/50 p-8 rounded-3xl border border-white/10 backdrop-blur-sm hover:border-indigo-500/50 transition-all group">
                                    ...
                                    <h3 className="text-slate-400">Total Operatives</h3>
                                    <p className="text-5xl font-bold text-white mt-2">{stats.totalUsers}</p>
                                </div>
                                <div className="bg-slate-900/50 p-8 rounded-3xl border border-white/10 backdrop-blur-sm hover:border-purple-500/50 transition-all group">
                                    <div className="flex justify-between items-start mb-4">
                                    ...
                                    <h3 className="text-slate-400">Active Cells</h3>
                                    <p className="text-5xl font-bold text-white mt-2">{stats.totalGroups}</p>
                                </div>
                                <div className="bg-slate-900/50 p-8 rounded-3xl border border-white/10 backdrop-blur-sm hover:border-emerald-500/50 transition-all group">
                                    <div className="flex justify-between items-start mb-4">
                                    ...
                                    <h3 className="text-slate-400">Total Intercepts</h3>
                    ...
                    {!loading && activeTab === 'users' && (
                        <div className="bg-slate-900/50 rounded-3xl border border-white/10 overflow-hidden backdrop-blur-sm">
                            {users.length === 0 ? (
                                <div className="p-12 text-center text-slate-500">No operatives found in the database.</div>
                            ) : (
                                <table className="w-full text-left">
                                    <thead className="bg-white/5 text-xs uppercase tracking-wider text-slate-400 font-medium">
                                        <tr>
                                    ...
                                    <tbody className="divide-y divide-white/5">
                                        {users.map(u => (
                                            <tr key={u.id} className="hover:bg-white/5 transition-colors">
                                                ...
                                                <td className="p-6 text-slate-400 font-mono text-sm">{u.email}</td>
                                                <td className="p-6">
                                                    <span className={`px-3 py-1 rounded-full text-xs font-bold ${u.role === 'ADMIN' ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 'bg-slate-700/50 text-slate-300 border border-slate-600/30'}`}>
                                                        {u.role || 'USER'}
                                                    </span>
                                                </td>
                            ...
                            >
                                ...
                                >
                                    <div className="p-6 border-b border-white/10 flex justify-between items-center bg-slate-800/50">
                                        ...
                                        <button onClick={() => setSelectedUser(null)} className="text-slate-500 hover:text-white text-2xl">×</button>
                                    </div>
                                    <div className="p-0 max-h-[60vh] overflow-y-auto">
                                        {userMessages.length === 0 ? (
                                            <div className="p-12 text-center text-slate-500 font-mono">NO INTERCEPTED COMMUNICATIONS</div>
                                        ) : (
                                            <table className="w-full text-left font-mono text-sm">
                                                <thead className="bg-black/20 text-xs text-slate-500 sticky top-0 backdrop-blur-md">
                                                    <tr>
                                                ...
                                                <tbody className="divide-y divide-white/5">
                                                    {userMessages.map(m => (
                                                        <tr key={m.id} className="hover:bg-white/5">
                                                            <td className="p-4 text-slate-500 whitespace-nowrap">{new Date(m.createdAt).toLocaleString()}</td>
                                    ...
                                    <div className="p-4 border-t border-white/10 bg-slate-800/50 flex justify-between items-center text-xs text-slate-500 font-mono">
                    ...
                    {!loading && activeTab === 'groups' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {groups.length === 0 ? <p className="text-slate-500">No active cells found.</p> : groups.map(g => (
                                <div key={g.id} className="bg-slate-900/50 p-6 rounded-2xl border border-white/10 hover:border-indigo-500/30 transition-all flex justify-between items-center group">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-slate-500 max-w-[200px] truncate">{g.description}</p>
                                        </div>
                                    </div>
                        ...
                        <div className="bg-black/50 rounded-2xl border border-white/10 overflow-hidden font-mono text-sm">
                            ...
                            {messages.map((m, idx) => (
                                <div key={`${m.type}_${m.id}_${idx}`} className="p-4 border-b border-white/5 flex gap-4 hover:bg-white/5 transition-colors group items-center">
                                    <div className="text-slate-500 w-24 shrink-0 text-xs">{new Date(m.createdAt).toLocaleTimeString()}</div>
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="text-[10px] text-slate-500 mt-1">{m.source}</div>
                                    </div>
                    ...
                    {!loading && activeTab === 'banned' && (
                        <div className="space-y-8">
                            <div className="bg-slate-900/50 p-6 rounded-2xl border border-white/10 flex gap-4">
                                <input
                                    type="text"
                                    value={newBannedWord}
                    ...
                    {!loading && activeTab === 'broadcast' && (
                        <div className="max-w-2xl mx-auto mt-10">
                            <div className="bg-slate-900 rounded-3xl border border-white/10 overflow-hidden shadow-2xl">
                                <div className="bg-slate-800/50 p-4 border-b border-white/5 flex items-center gap-2">
                                    <div className="w-3 h-3 rounded-full bg-red-500"></div>
                                    <div className="w-3 h-3 rounded-full bg-yellow-500"></div>
                                    <div className="w-3 h-3 rounded-full bg-green-500"></div>
                                    <span className="ml-4 text-xs font-mono text-slate-500">EMERGENCY_BROADCAST_SYSTEM</span>
                                </div>
                                <div className="p-8">
                                    <textarea
                                        value={broadcastMsg}
                            ...
                            <p className="text-center text-slate-500 mt-4 text-xs">WARNING: This message will be pushed to all active connected clients instantly.</p>
                        </div>
                    )}
                            ...
                            <div className="bg-slate-900/50 p-8 rounded-3xl border border-white/10">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-2 gap-4 mt-4">
                                        ...
                                        <div className="bg-black/30 p-4 rounded-xl border border-white/5">
                                            <p className="text-xs text-slate-500 uppercase">Active Threads</p>
